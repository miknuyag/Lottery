...
contract Lottery {
    struct BetInfo {
        ...
        address payable bettor; // 배팅한 사람 (주소)
        byte challenges; // 제시한 답
    }
    ...
    uint256 constant internal BET_BLOCK_INTERVAL = 1;
    ...
    event BET(uint256 index, address bettor, uint256 amount, byte challenges, uint answerBlockNumber);
     ...
     * 정답 실패 : 팟 머니 축적, 정답 맞춤 : 팟머니 획득, 한글자 맞춤 or 정답 확인 불가 : 배팅 금액만 획득
     */

    function distribute() public {
        // head 3 ..... 6 7 8 9 10 11 12 tail
        uint256 cur;
        ...
        for(cur=_head;cur<_tail;cur++) {
            b = _bets[cur]; // user가 배팅한 바이트 값을 b변수에 저장함 
            ...
            if(currentBlockStatus == BlockStatus.checkable) {
                ...
                if (currentBettingResult == BettingResult.Win) {
                    // transfer pot
                    transferAmount = transferAfterPayingFee(b.bettor, _pot + BET_AMOUNT);  //배팅한 금액까지 포함
            ...
            if(currentBlockStatus == BlockStatus.NotRevealed) { //마이닝 되지 않은 상태 - 반복문을 빠져나감. 현재 검증할 수 없는 블록(이후 동일)
                break;
            } 
    ...
    function transferAfterPayingFee(address payable addr, uint256 amount) internal returns (uint256) { 
        // 특정 addr(주소)에 amount(얼마)를 주겠다를 param으로 받고, 얼마에서 일정량의 수수료를 빼서 sc오너에게 주고 실제 전송될 분량을 return
        ...
        // call, send, transfer (ETH 전송하는 방법 *조심)
    ...
    function isMatch(byte challenges, bytes32 answer) public pure returns (BettingResult) {
        // challenges 0xab -> 첫번째, 두번째 글자를 뽑아서 challenges와 아래 answer에서 뽑은 글자와 비교하기.
        // answer 0xab......ff 32bytes

        byte c1 = challenges; //challenges에서 첫 번째 byte를 뽑아옴
        byte c2 = challenges;

        byte a1 = answer[0];
        byte a2 = answer[0];

        // Get first number (shifting 연산사용)
        c1 = c1 >> 4; // 0xab -> 0x0a
        c1 = c1 << 4; // 0x0a -> 0xa0
